...
export function AdminModal({ isOpen, onClose, onSubmit, actionText }: AdminModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!pin.trim()) { setError(true); return; }
    setError(false);
    onSubmit(pin);
    setPin('');
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm border border-gray-100 relative">
        ...
        <div className="flex flex-col items-center mb-6 mt-2">
          <div className="bg-red-50 p-3 rounded-full text-red-600 mb-3"><Lock size={24} /></div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button type="submit" className="w-full bg-[#003366] text-white font-bold py-3 rounded-xl hover:bg-[#002244] hover:shadow-lg transition-all active:scale-95">
            Autorizar Acción
          </button>
        </form>
      </div>
