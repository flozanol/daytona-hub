...
// --- CONFIGURACIÓN FINANCIERA (PLAN DE PISO AUTOS NUEVOS) ---
const TIIE = 6.7458;
const PUNTOS_ADICIONALES = 2;
const TASA_ANUAL = (TIIE + PUNTOS_ADICIONALES) / 100; // 0.087458
...
const CPNY_MAP: Record<string, { nombre: string, sector: string }> = {
  ...
  'GWIZT': { fontName: 'GWM Iztapalapa', sector: 'AUTOS' },
...
export default function ClinicaInventarioFinal() {
  ...
  const stats = useMemo(() => {
    ...
    let totCostoFinanciero = 0;
    
    dashboardData.forEach(d => {
      totInversion += Number(d.Costo) || 0;
      totCostoFinanciero += Number(d.CostoFinanciero) || 0;
    ...
    return {
      ...
      costoFinancieroTotal: totCostoFinanciero,
...
• Inversión total base: ${formatCurrencyM(stats.inversion)}
• Costo Financiero Total: ${formatCurrencyM(stats.costoFinancieroTotal)}
                      ...
                      return (
                        <div key={i} className="flex items-center gap-3 p-3 bg-slate-50 rounded-2xl border border-slate-100">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-slate-500 font-semibold">{formatCurrencyM(a.costo)} inmovilizado · {pct}% del inventario antiguo</p>
                          </div>
                        </div>
                      );
                    })}
                  ...
                  <table className="w-full text-sm text-left">
                    <thead className="bg-slate-50 text-[11px] uppercase text-slate-500 font-black border-b border-slate-200">
                      <tr>
                        ...
                        <th className="px-4 py-3 text-right">COSTO AUTO</th>
                        <th className="px-4 py-3 text-right text-purple-600">COSTO FINANCIERO</th>
                        <th className="px-4 py-3 text-right font-black text-red-600">INVERSIÓN TOTAL</th>
                          ...
                          <tr key={idx} className={`transition-colors ${aging.bg} hover:brightness-95`}>
                            ...
                            <td className="px-4 py-3">
                              <div className="flex flex-col gap-0.5">
                                ...
                                {row.Anio && <span className="text-[10px] font-medium text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded w-fit">M.Y. {row.Anio}</span>}
                              </div>
                            </td>
                ...
                <table className="w-full text-sm text-left">
                  <thead className="bg-slate-50/80 text-[11px] uppercase text-slate-500 font-black border-b border-slate-100 sticky top-0 z-10">
                    <tr>
                      ...
                      <th className="px-6 py-4 text-right">COSTO AUTO</th>
                      <th className="px-6 py-4 text-right text-purple-600">COSTO FIN.</th>
                      <th className="px-6 py-4 text-right font-black">INVERSIÓN TOTAL</th>
                    </tr>
                  </thead>
                    ...
                    {tableData.slice(0, 150).map((row, idx) => {
                      ...
                      return (
                        <tr key={idx} className={`transition-colors ${aging.bg} hover:brightness-95`}>
                          ...
                          <td className="px-6 py-4 text-right text-slate-500">{formatCurrency(row.Costo)}</td>
                          <td className="px-6 py-4 text-right text-purple-600 font-bold">{formatCurrency(row.CostoFinanciero)}</td>
                          <td className="px-6 py-4 text-right font-black text-slate-900 whitespace-nowrap">{formatCurrency(row.InversionTotal)}</td>
                        </tr>
                      );
                    })}
                  </tbody>
