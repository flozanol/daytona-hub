...
const Badge = ({ n }: { n: number }) => {
  ...
  if (n < 0) return <span className="inline-block bg-red-700 text-white text-xs font-black px-2.5 py-0.5 rounded-md">{n} SOBRA</span>;
  return <span className="inline-block bg-gray-400 text-white text-xs px-2.5 py-0.5 rounded-md">✔ OK</span>;
};
...
export default function YakimuraView() {
  ...
  const grupos = useMemo<VersionGroup[]>(() => {
    ...
    datosFiltrados.forEach(row => {
      ...
      if (!map[key]) {
        map[key] = { key, CpnyId: row.CpnyId, Marca: row.Marca, SubMarca: row.SubMarca, Version: row.Version, Anio: row.Anio, p3: 0, p2: 0, p1: 0, totalVentas: 0, promedio: 0, colores: [] };
      }
      ...
      map[key].colores.push({ Color: row.Color, p3: row.Periodo_Menos_3 ?? 0, p2: row.Periodo_Menos_2 ?? 0, p1: row.Periodo_Menos_1 ?? 0, ventas3m, promedio: ventas3m / 3 });
    });
  ...
  const exportarExcel = () => {
    const filas: string[][] = [];
    filas.push(['Agencia','Marca','Modelo','Versión','Año','Color','Mes -3','Mes -2','Mes -1','Total 3M','Promedio/mes','Pedir (inv=0)']);
    filtrados.forEach(g => {
      // Fila de versión
      filas.push([g.CpnyId, g.Marca, g.SubMarca, g.Version, String(g.Anio), 'TOTAL', String(g.p3), String(g.p2), String(g.p1), String(g.totalVentas), g.promedio.toFixed(1), String(calcComprar(g.promedio))]);
      // Filas de colores
      g.colores.sort((a,b) => b.ventas3m - a.ventas3m).forEach(c => {
        filas.push([g.CpnyId, g.Marca, g.SubMarca, g.Version, String(g.Anio), c.Color, String(c.p3), String(c.p2), String(c.p1), String(c.ventas3m), c.promedio.toFixed(1), String(calcComprar(c.promedio))]);
      });
    });
    const bom = '\uFEFF';
    const csv = bom + filas.map(f => f.map(v => `"${v.replace(/"/g,'""')}"`).join(',')).join('\n');
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center h-full py-24 text-gray-400">
      ...
      <p className="text-sm font-bold tracking-widest uppercase">Cargando datos de ventas...</p>
    </div>
  );
    ...
    <div className="p-6 md:p-8 max-w-screen-xl mx-auto">
      ...
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Pedido a planta — ventas últimos 3 meses vs. inventario</p>
        </div>
        <button
      ...
      <div className="flex flex-wrap gap-4 mb-6">
        {[
          ...
          { label: 'Total unidades a pedir', value: totalComprar, color: 'text-green-700' },
        ...
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr className="bg-[#003366] text-white">
              {['Agencia','Marca','Modelo','Versión','Año','Mes -3','Mes -2','Mes -1','Prom./mes','Pedir (inv=0)','Colores'].map(h => (
                <th key={h} className="px-3 py-3 font-bold whitespace-nowrap text-left first:text-left">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {filtrados.length === 0 && (
              <tr><td colSpan={11} className="py-16 text-center text-gray-400">No hay resultados.</td></tr>
            )}
            {filtrados.map((g, i) => {
              ...
              return (
                <>
                  <tr
                    key={g.key}
                    className={`border-b border-gray-100 transition-colors ${isOpen ? 'bg-indigo-50' : i % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-indigo-50`}
                  >
                    <td className="px-3 py-2.5 text-xs text-gray-500 font-mono">{g.CpnyId}</td>
                    <td className="px-3 py-2.5 font-bold text-[#003366]">{g.Marca}</td>
                    <td className="px-3 py-2.5 font-bold">{g.SubMarca}</td>
                    <td className="px-3 py-2.5 text-gray-600 max-w-[220px] truncate" title={g.Version}>{g.Version}</td>
                  ...
                  {isOpen && (
                    <tr key={`${g.key}-det`}>
                      <td colSpan={11} className="bg-indigo-50 px-6 pb-4 pt-1">
                        <table className="w-full text-xs border-collapse">
                          <thead>
                            <tr className="text-indigo-700 border-b-2 border-indigo-200">
                              {['🎨 Color','Mes -3','Mes -2','Mes -1','Total 3M','Prom./mes','% modelo','Pedir (inv=0)'].map(h => (
                            ...
                            {g.colores.slice().sort((a, b) => b.ventas3m - a.ventas3m).map(c => {
                              ...
                              return (
                                <tr key={c.Color} className="border-b border-indigo-100">
                                  ...
                                  <td className="px-2 py-1.5"><Badge n={calcComprar(c.promedio)} /></td>
                                </tr>
                              );
                            })}
                          </tbody>
      ...
      <p className="mt-3 text-xs text-gray-400">
        Datos en tiempo real · SQL Server · Base: Intranet · Vista: vw_VentasUltimos4Periodos
      </p>
    </div>
  );
}
