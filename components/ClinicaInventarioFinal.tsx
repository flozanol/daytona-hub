...
import { Upload, Search, Database, AlertCircle, TrendingUp, Filter, Clock, Pickaxe, BadgeDollarSign, Car, BarChart3 } from 'lucide-react';
...
export default function ClinicaInventarioFinal() {
  ...
  const cleanNumber = (val: any) => {
    if (!val) return 0;
    // Remueve comas, signos $, y espacios en blanco
      ...
      Papa.parse(file, {
        ...
        complete: (results) => {
          ...
          results.data.forEach((row: any) => {
            ...
            // REGLA CRÍTICA: Ignorar nulos y "Total" para no duplicar sumas
            if (colorStr !== '' && colorStr.toLowerCase() !== 'total' && colorStr.toLowerCase() !== 'sin clasificar') {
              ...
              parsedData.push({
                ...
                CostoTotalVal: costoTotalOriginal,
  ...
  const stats = useMemo(() => {
    ...
    dashboardData.forEach(d => {
      ...
      const rowTotalUnits = d.uFinanciados + d.uPropios + d.uDemos + d.uDemosPropios;
      // Usar el mayor de las unidades categorizadas o al menos 1
      const effectiveUnits = Math.max(rowTotalUnits, 1);
    ...
    return {
      ...
      totalInversion,
      edadPromedio,
      estancado: totMPropios + totMDemosPropios // Inversion en propios
    };
  }, [dashboardData]);
  ...
  const agingData = useMemo(() => {
    ...
    dashboardData.forEach(d => {
      ...
      const uds = (d.uFinanciados + d.uPropios + d.uDemos + d.uDemosPropios) || 1;
  ...
  // 4. RENDER VISUAL MODO "SEMINUEVOS" (Limpio, blanco, sombras suaves)
  return (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="bg-white p-6 rounded-[1.5rem] shadow-sm border border-slate-200 flex flex-col justify-between hover:shadow-md transition-shadow">
                <div className="flex items-center gap-3 text-slate-400 mb-2">
                  ...
                  <h3 className="font-bold uppercase tracking-wider text-[11px] text-slate-500">Total Unidades</h3>
                </div>
              ...
              <div className="bg-amber-50 p-6 rounded-[1.5rem] shadow-sm border border-amber-200 flex flex-col justify-between relative overflow-hidden group hover:shadow-md transition-shadow">
                ...
                <p className="text-3xl font-black text-amber-600 tracking-tight z-10 mx-[-2px]">{formatCurrencyM(stats.estancado)}</p>
                <div className="text-[10px] text-amber-800/70 mt-1 font-bold z-10 uppercase tracking-widest">
                  (Inversión en Propios)
                </div>
              </div>
            </div>

            {/* SECCIÓN INTERMEDIA: TABLA DE CAPITAL Y GRÁFICA DE AGING */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              
              {/* TABLA DE CAPITAL POR CATEGORÍA */}
              <div className="bg-white p-6 rounded-[1.5rem] shadow-sm border border-slate-200">
                <div className="flex items-center gap-2 mb-4">
                  <BarChart3 className="text-slate-400" size={20} />
                  <h3 className="text-slate-900 font-black text-lg">Capital por Categoría</h3>
                </div>
                <div className="overflow-hidden bg-slate-50 rounded-xl border border-slate-200 text-sm">
                  <table className="w-full text-left">
                    ...
                    <tbody className="divide-y divide-slate-200/60">
                      <tr className="hover:bg-white transition-colors">
                        <td className="px-4 py-3 font-bold text-slate-700">Financiados</td>
                        <td className="px-4 py-3 text-right font-semibold text-slate-600">{stats.financiados.uds}</td>
                        <td className="px-4 py-3 text-right font-black text-slate-800">{formatCurrency(stats.financiados.monto)}</td>
                      </tr>
                      <tr className="hover:bg-amber-50 transition-colors">
                        <td className="px-4 py-3 font-bold text-amber-800">Propios</td>
                        <td className="px-4 py-3 text-right font-semibold text-amber-700">{stats.propios.uds}</td>
                        <td className="px-4 py-3 text-right font-black text-amber-600">{formatCurrency(stats.propios.monto)}</td>
                      </tr>
                      <tr className="hover:bg-white transition-colors">
                        <td className="px-4 py-3 font-bold text-slate-700">Demos</td>
                        <td className="px-4 py-3 text-right font-semibold text-slate-600">{stats.demos.uds}</td>
                        <td className="px-4 py-3 text-right font-black text-slate-800">{formatCurrency(stats.demos.monto)}</td>
                      </tr>
                      <tr className="hover:bg-amber-50 transition-colors">
                        <td className="px-4 py-3 font-bold text-amber-800">Demos Propios</td>
                        <td className="px-4 py-3 text-right font-semibold text-amber-700">{stats.demosPropios.uds}</td>
                        <td className="px-4 py-3 text-right font-black text-amber-600">{formatCurrency(stats.demosPropios.monto)}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
              ...
              <div className="bg-white p-6 rounded-[1.5rem] shadow-sm border border-slate-200">
                <div className="flex justify-between items-center mb-6">
                  ...
                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider bg-slate-100 px-2 py-1 rounded-md">Días (Unidades)</span>
                </div>
                ...
                <table className="w-full text-sm text-left">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {tableData.slice(0, 150).map((row, idx) => {
                      ...
                      const isAlert = antiguedad > 90; // +90 marcado
                      ...
                      return (
                        <tr key={idx} className="hover:bg-slate-50/80 transition-colors group">
                          ...
                          <td className="px-6 py-4 text-center">
                            <span className={`font-bold ${isAlert ? 'text-red-600' : 'text-slate-700'}`}>
                              {antiguedad} <span className="text-xs font-medium text-slate-400">días</span>
                            </span>
                          </td>
