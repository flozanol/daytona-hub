...
export default function ClinicaSeminuevosSQL() {
      ...
      .then(sqlData => {
        if (Array.isArray(sqlData)) {
          const mapeado = sqlData.map((row: any) => {
            ...
            // Cálculo de Costo Financiero (Carrying Cost)
            // Fórmula: (Costo * Tasa Anual / 360 días) * Días de antigüedad
  ...
  // Función para obtener el color según antigüedad
  ...
  const exportToExcel = (rows: any[], filename: string) => {
    ...
    const exportData = rows.map(r => ({
        ...
        Días: r.Días,
        Costo: r.Costo,
        'Costo Financiero': Math.round(r.CostoFinanciero)
    }));
  ...
  if (loading) return <div className="p-20 text-center font-black animate-pulse text-[#003366]">GENERANDO AUDITORÍA FINANCIERA...</div>;
    ...
    <div className="min-h-full bg-slate-50 p-6 md:p-8 font-sans space-y-8">
      ...
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <KPICard title="Costo Financiero" value={fmtMoney(stats.financiero)} icon={<Coins />} color="text-amber-600" subtitle={`Tasa: ${(TASA_ANUAL*100).toFixed(2)}% Anual`} />
      ...
      <div className="bg-white rounded-3xl shadow-md border-2 border-red-500 overflow-hidden">
        <div className="bg-black p-4 flex justify-between items-center">
            <h2 className="text-white font-black text-sm uppercase flex items-center gap-2">
                <Skull className="text-red-500" size={18}/> Muro de los Lamentos (Pérdida por Estancamiento)
            </h2>
            ...
            <table className="w-full text-left text-xs">
                <thead className="bg-slate-50 font-black uppercase text-[10px] text-slate-500 border-b">
                    <tr>
                        ...
                        <th className="p-3 text-right bg-red-50 text-red-700">Gasto Financiero</th>
                    </tr>
                </thead>
      ...
      {/* INVENTARIO COMPLETO CON SEMÁFORO */}
      <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-6 border-b border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="flex gap-3 w-full md:w-auto">
                ...
                />
                <button onClick={() => exportToExcel(filteredData, 'Inventario_Completo_Daytona')} className="bg-emerald-600 text-white px-4 py-2 rounded-xl text-xs font-black uppercase">
                    Exportar Todo
                </button>
            </div>
        </div>
                ...
                <tbody className="divide-y divide-slate-100">
                    {filteredData.map((row, idx) => (
                        <tr key={idx} className="hover:bg-slate-50 transition-colors">
                            <td className="p-3 font-bold text-slate-500">{row.Sucursal}</td>
