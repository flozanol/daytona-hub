...
export async function GET() {
  try {
    ...
    const matched = inventory.filter((row: any) => salesKeys.has([norm(row.CpnyID), norm(row.Modelo), Number(row.ModelYr ?? row.Anio), norm(row.Color)].join('|')));
    const unmatched = inventory.filter((row: any) => !salesKeys.has([norm(row.CpnyID), norm(row.Modelo), Number(row.ModelYr ?? row.Anio), norm(row.Color)].join('|')));
