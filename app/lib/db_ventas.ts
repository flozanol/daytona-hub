...
export async function getVentasYakimura(): Promise<VentaRow[]> {
  ...
  try {
    ...
    const inventoryColumns = await poolInventory.request().query("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'Inventory'");
    const columnNames = inventoryColumns.recordset.map((r: any) => String(r.COLUMN_NAME));
    const pick = (names: string[]) => names.find(n => columnNames.some(c => c.toLowerCase() === n.toLowerCase()));
    const yearCol = pick(['Anio', 'Año', 'ModelYr', 'ModelYear', 'ANO']);
    const modelCol = pick(['Modelo', 'Model', 'ModelDescr']);
    const brandCol = pick(['BrandDescr', 'Marca', 'Brand']);
    const colorInvCol = pick(['Color', 'ColorDescr']);
    if (!yearCol || !modelCol || !brandCol || !colorInvCol) throw new Error(`Inventory columns missing: year=${yearCol}, model=${modelCol}, brand=${brandCol}, color=${colorInvCol}`);
    const invResult = await poolInventory.request().query(`SELECT LTRIM(RTRIM(CpnyID)) AS CpnyId, LTRIM(RTRIM([${brandCol}])) AS Marca, LTRIM(RTRIM([${modelCol}])) AS SubMarca, LTRIM(RTRIM(Version)) AS Version, [${yearCol}] AS Anio, LTRIM(RTRIM([${colorInvCol}])) AS Color, VIN, SUM(ISNULL(QtyAF, 0)) AS QtyAF, SUM(ISNULL(QtyAP, 0)) AS QtyAP, SUM(ISNULL(QtyAD, 0)) AS QtyAD, SUM(ISNULL(QtyDP, 0)) AS QtyDP FROM Inventory WHERE QtyAF > 0 OR QtyAP > 0 OR QtyAD > 0 OR QtyDP > 0 GROUP BY CpnyID, [${brandCol}], [${modelCol}], Version, [${yearCol}], [${colorInvCol}], VIN`);
    type InvRow = { CpnyId: string; Marca: string; SubMarca: string; Version: string; Anio: number; Color: string; VIN: string } & InventoryTotals;
    const inventory = invResult.recordset as InvRow[];
    return (ventasResult.recordset as Record<string, unknown>[]).map(v => { const candidates = inventory.filter(inv => norm(inv.CpnyId) === norm(v.CpnyId) && norm(inv.SubMarca) === norm(v.SubMarca) && Number(inv.Anio) === Number(v.Anio) && (!norm(v.Color) || norm(inv.Color) === norm(v.Color))); return normalizeRow({ ...v, QtyAF: candidates.reduce((n, inv) => n + inv.QtyAF, 0), QtyAP: candidates.reduce((n, inv) => n + inv.QtyAP, 0), Inventario: candidates.reduce((n, inv) => n + inv.QtyAF + inv.QtyAP + inv.QtyAD + inv.QtyDP, 0) }); });
