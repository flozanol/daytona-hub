...
export async function getVentasYakimura(): Promise<VentaRow[]> {
  ...
  try {
    ...
    return (ventasResult.recordset as Record<string, unknown>[]).map(v => {
      const candidates = inventory.filter(inv => norm(inv.CpnyId) === norm(v.CpnyId) && norm(inv.SubMarca) === norm(v.SubMarca) && Number(inv.Anio) === Number(v.Anio) && (!norm(v.Color) || norm(inv.Color) === norm(v.Color)));
      return normalizeRow({ ...v, QtyAF: candidates.reduce((n, inv) => n + inv.QtyAF, 0), QtyAP: candidates.reduce((n, inv) => n + inv.QtyAP, 0), Inventario: candidates.reduce((n, inv) => n + inv.QtyAF + inv.QtyAP + inv.QtyAD + inv.QtyDP, 0) });
    });
  } finally {
